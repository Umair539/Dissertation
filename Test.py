...
stats = pd.read_csv("stats.csv")  # using normalization values used on training data
a, b = stats.iloc[-1]
...
for t in [1, 5]:
    ...
    for i in range(2):
        ...
        if i == 0:
            ...
            model.summary()
        if i == 1:
            model = keras.models.load_model("Final_Non_Linear_" + str(t) + ".keras")
            model.summary()
        ...
        if i == 0:
            ...
            ax.legend()
            fig.savefig("Linear Model Predictions for t + " + str(t))

        if i == 1:
            ax.set_title("Non-Linear Model Predictions for t + " + str(t))
            ax.legend()
            fig.savefig("Non-Linear Model Predictions for t + " + str(t))
